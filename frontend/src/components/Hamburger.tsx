...
export function SideBar({
  ...
  useEffect(() => {
    axios
      .get("https://survey-poll-backend.onrender.com/api/v1/me", {
        withCredentials: true,
      })
      .then((res) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 border-b-2 ">
        {auth ? (
          <>
            <div className="w-16 h-16 rounded-full bg-black dark:bg-btncolor flex items-center justify-center ">
          ...
          <span className="text-xl font-bricolage font-semibold text-btncolor">
            Please Signup
          </span>
        )}
      </div>
        ...
        <ul className="flex flex-col">
          ...
          {auth ? (
            <>
              <Link
                to={"/post"}
                className="py-2 text-xl font-semibold font-bricolage  hover:text-btncolor"
              >
                {" "}
                Create Survey
              </Link>
